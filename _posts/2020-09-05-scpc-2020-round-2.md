...
때문에, 펜윅트리를 버킷마냥 여러 개로 쪼개서 관리하면 로그항의 시간복잡도도 낮출 수 있을 뿐더러 cache miss도 덜 납니다. 저는 $2^{14}$ 단위로 끊어서 관리했습니다. 이러면 수의 갱신은 $\mathcal{O}(\lg 2^{14})$, 중앙값 계산은 $\mathcal{O}(\sqrt{\frac{KN}{2^{14}}} + \lg 2^{14})$ 정도에 진행할 수 있어 총 시간 복잡도는 $\mathcal{O}(Q(K \lg 2^{14} + \sqrt{\frac{KN}{2^{14}}} + \lg 2^{14}))$, 공간 복잡도는 $\mathcal{O}(KN)$이 됩니다. 2초 이내에 돌았던 것으로 기억합니다.
